    ...
    $(function () {
        $('#tp_clo_w_1').highcharts({
            ...
            xAxis: {
                categories: [
                    <?php
                    $sql = "SELECT `Etat` as 'etat' FROM swan where (`fin` >= CURRENT_DATE - INTERVAL 7 DAY) and (`fin` <= CURRENT_DATE) and `Etat` NOT LIKE 'TERMINE%' GROUP BY `Etat` ";
            ...
            series: [
                ...
                while($row2 = mysql_fetch_array($result2)) {
                    ...
                    while($row = mysql_fetch_array($result)) {
                        ...
                        $sql3 = "SELECT COUNT(*) AS 'number' FROM swan where (`fin` >= CURRENT_DATE - INTERVAL 7 DAY) and (`fin` <= CURRENT_DATE) and `Etat` LIKE '".$user."' and `Demandeur` LIKE '".$inci."' ";
